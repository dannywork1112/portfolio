...
namespace Zeus
{
    public class QuestManager : UnitySingleton<QuestManager>
    {
        ...
        public void RemoveQuest(QuestProcess process, List<QuestProcess> targetProcessList = null)
        {
            ...
            var questTable = TableManager.GetQuestTable();
            var curPlayer = TableManager.CurrentPlayerData;
            var curQuest = questTable.GetData(processID);
            ...
            if (process.IsDone)
            {
                curPlayer.ClearQuest(processID);
                RewardManager.GetReward(processID);
            }
            else
                curPlayer.RemoveQuest(processID);

            // 하위퀘스트 삭제
            // NOTE : 하위퀘스트의 하위퀘스트도 존재? 해당경우 DependenceProcess에 기록?
            // 위의 경우 리스트가 꼬일 수 있음. 다른 방법이 필요함.
            // dependence의 depth를 기록?
            var dependenceQuests = questTable.Quests.Where(x => x.DependenceQuestID == processID).ToList();
