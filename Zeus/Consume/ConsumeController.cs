...
namespace Zeus
{
    public class ConsumeController : MonoBehaviour
    {
        ...
        private void OnEnter()
        {
            ...
            // 쿨다운여부
            var cooltime = GameTimeManager.Instance.GetCoolTime(tableID) as CoolTimer;
        ...
        public void ConsumeEffectEnter()
        {
            ...
            var consumeUI = PlayerUIManager.Get().GetUI(TypePlayerUI.CONSUMABLE) as PlayerConsumeUI;
            if (consumeUI != null)
            {
                var consumeData = playerData.GetConsumeData(_curConsumeData.ID);
                new CoolTimer(_curConsumeData.ID, _curConsumeData.Cooldown);
                consumeUI.RefreshConsume(consumeData.Amount > 0);
            }

            QuickTabSceneUIManager.Get()?.RefreshCurrentTab();
        }
